...
def plt_g(df, x, y_b, y_l, tit):
    fig = make_subplots(specs=[[{"secondary_y": True}]])

    fig.add_trace(go.Bar(name=y_b, x=df[x], y=df[y_b], marker_color='#E97132'), secondary_y=False)
    fig.add_trace(go.Scatter(name=y_l, x=df[x], y=df[y_l], mode='lines', line=dict(color='#111E6C', width=3)),
                  secondary_y=True)
...
# --- Organização das Duas Abas Finais ---
t1, t2 = st.tabs([
    "📊 Dashboard & Estatísticas",
    "🔍 Gráficos de Exploração (EDA)"
])

# --- ABA 1: Visuais e Dados Consolidados ---
with t1:
    ...
    # Aplicação do filtro reativo nas tabelas temporárias
    ...
    g1, g2 = st.columns(2)
    with g1:
        f_h = plt_g(d_af, 'Ano', 'Chuva_mm', 'Preco_Real', "Evolução Histórica")
    ...
    with g2:
        ...
        f_s = plt_g(d_sm, 'Mes', 'Chuva_mm', 'Preco_Real', "Sazonalidade Mensal")
    ...
    # Tabelas Estatísticas integradas no rodapé da mesma página
...
# --- ABA 2: Gráficos Fixos da Análise Exploratória (EDA) ---
with t2:
